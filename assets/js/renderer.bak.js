...
function renderProfilesList() {
    ...
    if (profilesList.length === 0) {
        container.innerHTML = '<p style="color: var(--text-muted); font-style: italic;">Chưa có Profile nào �ược lưu.</p>';
        return;
    }
    ...
    profilesList.forEach((profile, idx) => {
        ...
        card.innerHTML = `
            ...
            <div style="font-size: 0.9rem; color: var(--text-muted);">
                <p style="margin: 0;">Quản lý: <strong>${linksCount}</strong> Nơi �Ēng (Fanpage/Group)</p>
            </div>
            <div style="margin-top: 5px;">
                <button class="btn btn-primary btn-open-profile-direct" data-idx="${idx}" style="width: 100%; font-size: 0.9rem; padding: 8px;">M�x Trình Duy�!t ${profile.profileName}</button>
            </div>
        `;
        container.appendChild(card);
    });

    // Gắn sự ki�!n cho các nút Edit / Delete / Open
    ...
    container.querySelectorAll('.btn-delete-profile').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const idx = e.target.getAttribute('data-idx');
            if (confirm("Chắc chắn mu�n xóa Profile này khỏi danh sách? Dữ li�!u Trình duy�!t n�"i b�" Playwright sẽ giữ nguyên, ch�0 xóa trên Tool.")) {
    ...
    container.querySelectorAll('.btn-open-profile-direct').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            try {
                ...
                alert("Không thỒ m�x trình duy�!t: " + err);
            }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(btnSaveProfile) {
        btnSaveProfile.addEventListener('click', () => {
            ...
            if(!pName || pName === '') return alert("Vui lòng nhập tên Profile!");
...
btnParseContent.addEventListener('click', async () => {
    ...
    if (activeTab === 'paste') {
        ...
        if(!text) return alert("Vui lòng nhập n�"i dung!");
        
        // Tách bằng 2 dòng tr�ng
        contentsList = text.split(/\n\s*\n/).filter(t => t.trim() !== '');
        alert(`Đã nạp ${contentsList.length} n�"i dung từ Text.`);
    } else {
        ...
        if(file.name.endsWith('.txt')) {
            ...
            contentsList = text.split(/\n\s*\n/).filter(t => t.trim() !== '');
            alert(`Đã nạp ${contentsList.length} n�"i dung từ File Text.`);
            ...
            alert(`Đã nạp ${contentsList.length} n�"i dung từ File Excel.`);
        }
    }
});
...
btnSelectVideo.addEventListener('click', async () => {
    ...
    if(dirPath) {
        ...
        if(videos.error) {
            alert("L�i �ọc thư mục: " + videos.error);
            return;
        }
        videosList = videos;
...
btnGenerate.addEventListener('click', () => {
    if(contentsList.length === 0) return alert("Chưa có n�"i dung. Vui lòng phân tích content trư�:c!");
    if(videosList.length === 0) return alert("Chưa có video. Vui lòng chọn thư mục video!");
...
if (btnStartInteraction) {
    btnStartInteraction.addEventListener('click', async () => {
        ...
        if (!apiKey) {
            return alert('Vui lòng nhập Gemini API Key �Ồ cấu hình AI comment!');
        }
        if (profilesList.length === 0) {
            return alert('Chưa có Profile/Nơi �Ēng nào �ược thiết lập. Vui lòng thêm Profile �x tab Quản lý Profile!');
        }
        ...
        try {
            ...
            alert("Đã kết thúc phiên tương tác dạo!");
        } catch (e) {
            console.error(e);
            if(e.message && !e.message.includes("Interaction stopped by user")) {
                alert("Có l�i xảy ra: " + e.message);
            }
        } finally {
            // Restore UI
...
if (btnStopInteraction) {
    btnStopInteraction.addEventListener('click', async () => {
        const confirmStop = confirm("Bạn có chắc chắn mu�n dừng hẳn tiến trình tương tác không?");
        if(confirmStop) {
...
if (toggleApiBtn) {
    toggleApiBtn.addEventListener('click', () => {
        const apiEl = document.getElementById('api-gemini-key');
        if (apiEl.style.webkitTextSecurity === 'none') {
            apiEl.style.webkitTextSecurity = 'disc';
        } else {
            apiEl.style.webkitTextSecurity = 'none';
        }
    });
}

// TÍNH N�NG THEO D�"I TIẾN Đ�� TƯƠNG TÁC
let currentProgressionIndex = 0;

function renderInteractionProgressGrid() {
    const grid = document.getElementById('interaction-profile-status-grid');
    if (!grid) return;
    grid.innerHTML = '';
    
    profilesList.forEach((row, idx) => {
        const tag = document.createElement('span');
        tag.className = 'profile-tag';
        if (idx < currentProgressionIndex) {
            tag.classList.add('profile-tag-done');
        } else {
            tag.classList.add('profile-tag-pending');
        }
        tag.innerText = row.profileName;
        grid.appendChild(tag);
    });
}

// Bắt sự ki�!n hoàn thành profile
if (window.api && window.api.onInteractionProfileComplete) {
    window.api.onInteractionProfileComplete(async (pIdx) => {
        currentProgressionIndex = pIdx + 1;
        config.interactionSavedIndex = currentProgressionIndex;
        await window.api.saveConfig(config);
        renderInteractionProgressGrid();
    });
}

// Khi load config xong
setTimeout(() => {
    if (config && config.interactionSavedIndex) {
        currentProgressionIndex = config.interactionSavedIndex;
    }
    renderInteractionProgressGrid();
}, 1500);
